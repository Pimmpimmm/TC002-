...
- TC002 显示 `device not found` 或 `No route to host`：地址栏只填纯 `IPv4` 或 `IPv4:5555`，不要粘贴网页链接。在 macOS“系统设置 → 隐私与安全性 → 本地网络”允许 TC002 专注助手访问局域网；时钟需开启 Wi-Fi ADB，并核对它当前的 IP。macOS 15+ 会单独管理 App 的本地网络权限。
